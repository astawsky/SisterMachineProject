...
def pooling(number, cycle, trace, A_dict, B_dict, x_avg, disp):
    ...
    if number == 1:
        ...
        A_arrays = np.array([np.array([np.log(A_dict[idA][param][cyc + dispA] / x_avg) for cyc in
                                       range(min(len(A_dict[idA][param]) - dispA, len(B_dict[idB][param]) - dispB))])
        ...
        # Since either dispA or dispB is equal to 0, cycle + dispA + dispB = cycle + disp
        A_trace = np.array([A_dict[idA]['growth_length'][cycle + dispA] * A_dict[idA]['generationtime'][cycle + dispA]
                            for idA, idB in zip(A_dict.keys(), B_dict.keys())
                            if min(len(A_dict[idA]), len(B_dict[idB])) > cycle + dispA + dispB])

        B_trace = np.array([B_dict[idB]['growth_length'][cycle + dispB] * B_dict[idB]['generationtime'][cycle + dispB]
                            for idA, idB in zip(A_dict.keys(), B_dict.keys())
                            if min(len(A_dict[idA]), len(B_dict[idB])) > cycle + dispA + dispB])
...
def scatterPlot(A_trace, B_trace, xlabel, ylabel, trace, PoolID, cycle, disp, dset):
    ...
    plt.xlim([.3, 1.3])
    plt.legend()
    plt.savefig(dset + ' Scatter Plots ' + direct + ' cycle: ' + str(
        cycle) + ', disp: ' + str(disp))
    plt.close()
...
def GenerationPlot(A_GenPlotArray, B_GenPlotArray, xlabel, trace, PoolID, disp, dset):
    ...
    plt.ylim([-1, 1])
    plt.savefig(dset + direct + ' Pearson Coeff ' + ', disp = ' + str(disp))
    plt.close()
    ...
    plt.ylim([-1, 1])
    plt.savefig(dset + direct +' Slope and Intercept ' + ', disp = ' + str(disp))
    plt.close()
        ...
        for trace in ['A', 'B', 'Itself']:
            ...
            for cycle in range(how_many_cycles):
                ...
                for PoolID in [3]:  # [1,2,3]
                    ...
                    #             ylabel=r'$ln(\frac{x^B}{x^*})$', trace=trace, PoolID=PoolID, cycle=cycle, disp=disp,
                    #             dset="Sisters")
            ...
            if PoolID == 3:
                ...
                GenerationPlot(A_GenPlotArray=GenPlotArray_non_sis_A, B_GenPlotArray=GenPlotArray_non_sis_B,
                               xlabel='Generation'
                               , trace=trace, PoolID=3, disp=disp, dset="Non")
                GenerationPlot(A_GenPlotArray=GenPlotArray_both_A, B_GenPlotArray=GenPlotArray_both_B,
                               xlabel='Generation'
