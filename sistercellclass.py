...
class SisterCellData(object):
    ...
    def CellDivisionTrajectory(self,dataID, meth, discretize_by = 'length', sisterdata = None, additional_columns = []):
        ...
        for ks in keysuffix:
            ...
            # index_div = []
                ...
                #     point = diff.index
                #     if point < wind_width:
                #         window = abs(diffdata[0:(point+windwidth)])
                #     if point > wind_width and point < (len(diffdata)-wind_width):
                #         window = abs(diffdata[point-wind_width:point+wind_width])
                #     if point > (len(diffdata)-wind_width):
                #         window = abs(diffdata[point - wind_width:len(diffdata)])
                #     # IF THE LARGEST DIFFERENCE IS 3 TIMES LARGER THAN THE 2ND LARGEST IN THE WINDOW, THEN IT IS A JUMP
                #     if window.list()[0] > window.list()[1]:
                #         index_div.append()
                # index_div = np.where(diffdata > thresh)[0].flatten()
            ...
            # ret_ks['alphas'] = ( np.log(ret_ks[discretize_by + '_final']) - np.log(ret_ks[discretize_by + '_birth']) ) \
            #                    / ret_ks[discretize_by + '_birth']

            # # FIT THE ALPHAS FOR EACH CYCLE
            # alphas_array = []
            # range_array = []
            # points_array = []
            # for start, end in zip(start_times, end_times):
            #     reg = LinearRegression(fit_intercept=True, normalize=False, copy_X=False).fit(
            #         np.array(range(end - start))[:, np.newaxis], np.log(self.__data[dataID][discretize_by + ks][start:end]))
            #     alphas_array.append(reg.coef_)
            #     range_array.append(np.array(range(end - start))[:, np.newaxis])
            #     points_array.append(np.log(self.__data[dataID][discretize_by + ks][start:end]))
            #     # alphas_array.append(optimization.curve_fit(fit_func, xdata=np.array(range(start, end)),
            #     #                                             ydata=self.__data[dataID][discretize_by + ks][start:end]))
            # alphas_array = np.array(alphas_array).flatten()
            # ret_ks['alphas'] = alphas_array
            ...
            for final, next_beg in zip(ret_ks[discretize_by + '_final'][0:-1], ret_ks[discretize_by + '_birth'][1:]):
                div_rats.append((final-next_beg)/final)
            div_rats.append(ret_ks[discretize_by + '_final'][-1] - self.__data[dataID][discretize_by + ks]
                                                                                        [index_div[-1]+1])
            ret_ks['division_ratios__f_n'] = div_rats

            # print('alphas',len(alphas_array), 'gentime',len(ret_ks['generationtime']),'final', len(ret_ks[discretize_by
            # + '_final']), 'birth', len(ret_ks[discretize_by + '_birth']), 'f_n', len(ret_ks['division_ratios__f_n']),
            #       'growth',len(ret_ks['growth_' + discretize_by]) )
    ...
    def AutocorrelationFFT(self, dataID, normalize = False, maxlen = None, enforcelen = False) :
        ...
        """
        trajA, trajB = self.CellDivisionTrajectory(dataID, meth='threshold')
...
class CycleData(SisterCellData):
    def __init__(self, **kwargs):
        ...
        # Both1 = SisterCellData(infiles = random.choices(infiles_sisters + infiles_nonsisters, k =
        #                 min(len(infiles_sisters), len(infiles_nonsisters))), debugmode=False)
        ...
        for dataID in range(0, len(infiles_sisters), 1):
            ...
            self.dict_with_all_sister_traces = self.Sisters.DictionaryOfSisterTraces(dataID, meth, discretize_by='length',
        ...
        for dataID in range(0, len(infiles_nonsisters) - 1, 1):
            self.dict_with_all_non_sister_traces = self.Nonsisters.DictionaryOfSisterTraces(dataID, meth, discretize_by='length',
                                                                                  dictionary=self.dict_with_all_non_sister_traces,
                                                                                  sis='No')
            if dataID == 0:
                del self.dict_with_all_non_sister_traces['none']

        # self.dict_with_all_both_traces = {'none': 1}
        # for dataID in range(0, len(infiles_nonsisters) - 1, 1):
        #     self.dict_with_all_both_traces = self.Both.DictionaryOfSisterTraces(dataID, meth,
        #                                                                                     discretize_by='length',
        #                                                                                     dictionary=self.dict_with_all_both_traces,
        #                                                                                     sis='Both')
        #     if dataID == 0:
        #         del self.dict_with_all_both_traces['none']
        ...
        self.x_avg = xstar1(self.dict_all)
        ...
        self.A_dict_non_sis = dict(zip([k for k, v in self.dict_with_all_non_sister_traces.items() if 'Non-sister_Trace_A_' in k],
        ...
        self.B_dict_non_sis = dict(zip([k for k, v in self.dict_with_all_non_sister_traces.items() if 'Non-sister_Trace_B_' in k],
                                  [v for k, v in self.dict_with_all_non_sister_traces.items() if
                                   'Non-sister_Trace_B_' in k]))
        # self.A_dict_both = dict(
        #     zip([k for k, v in self.dict_with_all_both_traces.items() if 'Both_Trace_A_' in k],
        #         [v for k, v in self.dict_with_all_both_traces.items() if
        #          'Both_Trace_A_' in k]))
        # self.B_dict_both = dict(zip([k for k, v in self.dict_with_all_both_traces.items() if 'Both_Trace_B_' in k],
        #                            [v for k, v in self.dict_with_all_both_traces.items() if 'Both_Trace_B_' in k]))
        ...
        #
        # both_keysA = random.sample([k1 for k1 in self.dict_with_all_sister_traces.keys()], k=87)
        #
        # both_valuesA = [self.dict_all[ky] for ky in both_keysA]
        #
        # self.A_dict_both = dict(zip(both_keysA, both_valuesA))
        #
        # both_keysB = random.sample([k2 for k2 in self.dict_with_all_non_sister_traces.keys()], k=87)
        #
        # both_valuesB = [self.dict_all[ky] for ky in both_keysB]
        #
        # print('keys',len(both_keysB), 'values', len(both_valuesB))
        #
        # self.B_dict_both = []
        #
        # self.B_dict_both = dict(zip(both_keysB, both_valuesB))
            ...
            if 's' in dsets:
                ...
                if graph == True:
                    ...
                    for param in pval_sis[list(pval_sis.keys())[0]]:
                        ...
                        plt.legend()
                        plt.savefig('/Users/alestawsky/PycharmProjects/untitled/pearsonoverindividual/Sisters/'+str(param))
                        plt.close()
                    ...
                    for param in pval_sis[list(pval_sis.keys())[0]]:
                        ...
                        plt.title('Sister for ' + param)
                        plt.legend()
                        plt.savefig('/Users/alestawsky/PycharmProjects/untitled/pearsonoverindividual/Sisters/'+'pval, '+str(param))
                        plt.close()
            ...
            if 'n' in dsets:
                ...
                if graph == True:
                    ...
                    for param in pval_non[list(pval_non.keys())[0]]:
                        ...
                        plt.legend()
                        plt.savefig('/Users/alestawsky/PycharmProjects/untitled/pearsonoverindividual/Non/'+str(param))
                        plt.close()
                    ...
                    for param in pval_non[list(pval_non.keys())[0]]:
                        ...
                        plt.title('Non-Sister for ' + param)
                        plt.legend()
                        plt.savefig('/Users/alestawsky/PycharmProjects/untitled/pearsonoverindividual/Non/'+'pval, '+str(param))
                        plt.close()
                ...
                if cloud == True:
                    ...
                    for param in self.A_dict_non_sis[keyA].keys():
                        ...
                        plt.figure()
                        plt.scatter(self.A_dict_non_sis[keyA][param][:minlen], self.B_dict_non_sis[keyB][param][:minlen])
        ...
        #             plt.show()
            ...
            if 's' in dsets:
                ...
                for param in self.A_dict_sis[list(self.A_dict_sis.keys())[0]].keys():
                    ...
                    for param in pop_sis_param.keys():
                        ...
                        plt.title(param)
                        plt.savefig(
                            '/Users/alestawsky/PycharmProjects/untitled/pearsonoverset_timegraph/Sisters/populations_' + str(param),
                            dpi=300)
                        plt.close()
                ...
                if graph == True:
                    ...
                    for param in pval_sis_param.keys():
                        ...
                        plt.title('Sister (up to 20 gens) for ' + param)
                        plt.legend()
                        plt.savefig('/Users/alestawsky/PycharmProjects/untitled/pearsonoverset/Sisters/'+'pval, '+str(param),
                                    dpi=300)
                        plt.close()
                ...
                if time_graph == True:
                    ...
                    for param in pval_sis_param.keys():
                        ...
                        plt.title(r'Sister ' + param + r', $\mu=$' + '{:.2e}'.format(np.mean(dist_of_pairs))
                                       + r', $\sigma=$' + '{:.2e}'.format(np.std(dist_of_pairs)))
                        plt.savefig('/Users/alestawsky/PycharmProjects/untitled/pearsonoverset_timegraph/Sisters/'+str(param),
                                    dpi=300)
                        plt.close()
                    ...
                    for param in pval_sis_param.keys():
                        ...
                        plt.title(r'Sister ' + param + r', $\mu=$' + '{:.2e}'.format(np.mean(dist_of_pairs))
                                  + r', $\sigma=$' + '{:.2e}'.format(np.std(dist_of_pairs)))
                        plt.savefig('/Users/alestawsky/PycharmProjects/untitled/pearsonoverset_timegraph/Sisters/'+'pval, '+str(param),
                                    dpi=300)
                        plt.close()
            ...
            if 'n' in dsets:
                ...
                for param in self.A_dict_non_sis[list(self.A_dict_non_sis.keys())[0]].keys():
                    p_val_cycle = dict()
                    longest_cycle_A = max([len(self.A_dict_non_sis[keyA][param]) for keyA in self.A_dict_non_sis.keys()])
                    longest_cycle_B = max([len(self.B_dict_non_sis[keyB][param]) for keyB in self.B_dict_non_sis.keys()])
                    longest_cycle = min(longest_cycle_A, longest_cycle_B)
                    for cycle in range(longest_cycle):
                        sample1 = np.array([])
                        sample2 = np.array([])
                ...
                if graph == True:
                    ...
                    for param in pval_non_param.keys():
                        ...
                        plt.title('Non Sisters (up to 20 gens) for ' + param)
                        plt.legend()
                        plt.savefig('/Users/alestawsky/PycharmProjects/untitled/pearsonoverset/Non/'+'pval, '+str(param),
                                    dpi=300)
                        plt.close()
                ...
                if time_graph == True:
                    ...
                    for param in pval_non_param.keys():
                        ...
                        plt.title(r'Non Sister ' + param + r', $\mu=$' + '{:.2e}'.format(np.mean(dist_of_pairs))
                                       + r', $\sigma=$' + '{:.2e}'.format(np.std(dist_of_pairs)))
                        plt.savefig('/Users/alestawsky/PycharmProjects/untitled/pearsonoverset_timegraph/Non/'+str(param),
                                    dpi=300)
                        plt.close()
                    ...
                    for param in pval_non_param.keys():
                        ...
                        plt.title(r'Non Sister ' + param + r', $\mu=$' + '{:.2e}'.format(np.mean(dist_of_pairs))
                                  + r', $\sigma=$' + '{:.2e}'.format(np.std(dist_of_pairs)))
                        plt.savefig('/Users/alestawsky/PycharmProjects/untitled/pearsonoverset_timegraph/Non/'+'pval, '+str(param),
                                    dpi=300)
                        plt.close()
            ...
            if 'b' in dsets:
                ...
                if graph == True:
                    ...
                    for param in pval_both_par.keys():
                        ...
                        plt.legend()
                        plt.savefig(
                            '/Users/alestawsky/PycharmProjects/untitled/pearsonoverset/Control/' + 'pval, ' + str(param),
                            dpi=300)
                        plt.close()
    ...
    def PearsonPooled(self):
        self.both_pooled_data_dict['columns over pool cycle']
    ...
    def CompMeans(self):
        ...
        for cycle_param in self.A_dict_sis['Sister_Trace_A_0'].keys():
            some.update({cycle_param: np.mean(np.abs([np.mean(self.A_dict_sis[TrajA][cycle_param])
                                                 ...
                                                 zip(list(self.A_dict_sis.keys())[:87], self.B_dict_sis.keys())])) })
        other = dict()
        for cycle_param in self.A_dict_non_sis['Non-sister_Trace_A_0'].keys():
            other.update({cycle_param: np.mean(np.abs([np.mean(self.A_dict_non_sis[TrajA][cycle_param])
                                                 -np.mean(self.B_dict_non_sis[TrajB][cycle_param]) for TrajA, TrajB in
        ...
        for cycle_param in self.A_dict_sis['Sister_Trace_A_0'].keys():
            some1.update({cycle_param: np.std(np.abs([np.mean(self.A_dict_sis[TrajA][cycle_param])
                                               - np.mean(self.B_dict_sis[TrajB][cycle_param]) for TrajA, TrajB in
                                               zip(list(self.A_dict_sis.keys())[:87], list(self.B_dict_sis.keys())[:87])]))})
        other1 = dict()
        for cycle_param in self.A_dict_non_sis['Non-sister_Trace_A_0'].keys():
    ...
    # def __getattr__(self, key):
    #     if key == 'A_dict_non_sis':
    #         return self.A_dict_non_sis
